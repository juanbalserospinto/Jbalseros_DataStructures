...
static bool ReadYesNo(const string& prompt, bool* yes) {
  if (!yes) return false;
  cout << prompt;
  string s;
  if (!getline(cin, s)) return false;
  ...
  if (c == 'Y') { *yes = true; return true; }
  if (c == 'N') { *yes = false; return true; }
  return false;
}
...
int main(int argc, char** argv) {
  ...
  // Helpful coordinate examples (shown once)
  ...
  while (run_again) {
    ...
    size_t k = 0;
    if (!ReadSizeT("how many nearest stadiums you want to be shown: ", &k) || k == 0) {
